...
describe('EdgeSpeechTTS audio cache', () => {
  test('coalesces concurrent requests for the same audio payload', async () => {
    ...
    vi.stubGlobal('URL', {
      ...URL,
      createObjectURL: vi.fn(() => 'blob:coalesced-audio'),
    ...
    try {
      ...
      const payload = {
        lang: 'en-US',
        text: `concurrent-${Date.now()}`,
        voice: 'en-US-AriaNeural',
        rate: 1,
        pitch: 1,
      };

      const first = tts.createAudio(payload);
      const second = tts.createAudio(payload);
      ...
      await expect(Promise.all([first, second])).resolves.toEqual([
        { url: 'blob:coalesced-audio', boundaries: [] },
        { url: 'blob:coalesced-audio', boundaries: [] },
      ]);
